...
const PdfModal: React.FC<Props> = ({ isOpen, onClose, pdfUrl, title }) => {
  useEffect(() => {
    if (!isOpen) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-5xl overflow-hidden rounded-2xl border border-pink-200 bg-white/90 shadow-xl backdrop-blur-md">
        ...
        <iframe src={pdfUrl} title={title} className="h-[75vh] w-full" />
      </div>
    </div>
  );
};
